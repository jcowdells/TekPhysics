...
int render() {
    ...
    // add a framebuffer callback to allow for resizing the window
    tekAddFramebufferCallback(tekMainFramebufferCallback);

    // create a font to render the program name + version
    ...
    // once font has been used, we can delete the font creator
    tekDeleteFreeType();

    // some OpenGL stuff for blending and shading etc.
    ...
    // the mesh data for the walls/floor
    TekMesh walls = {};
    ...
    // mesh data for the cube (well, half of a cube that's visible)
    TekMesh cube = {};
    ...
    // creating the projection matrix for the shader
    glm_perspective(1.2f, width / height, 0.1f, 100.0f, perp_projection);
    ...
    // keeping track of time to maintain cube moving at constant speed.
    struct timespec t;
    ...
    while (tekRunning()) {
        ...
        // update matrices to cause movement of cube
        tekBindShaderProgram(shader_program);
        ...
        // move cube forward depending on whether or not it has already "hit the wall"
